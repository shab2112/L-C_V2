...
function ControlTray({ trayRef }: ControlTrayProps) {
  ...
  const micButtonTitle = muted ? 'Enable Microphone' : 'Disable Microphone';

  const connectButtonTitle = connected ? 'End Session' : 'Start New Session';

  return (
    <section className="control-tray" ref={trayRef}>
      <nav className={cn('actions-nav', { 'text-entry-visible': isTextEntryVisible })}>
        <button
        ...
        <button
          ...
          title={!speakerMuted ? 'Mute Speaker' : 'Unmute Speaker'}
        >
        ...
        <button
          ...
          title={isTextEntryVisible ? 'Hide Text Input' : 'Show Text Input'}
        >
          <span className="icon">
