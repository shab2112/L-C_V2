...
export default function StreamingConsole() {
  ...
  // Persistence logic: Save new turns to DB
  useEffect(() => {
    const user = useAuthStore.getState().user;
    const { sessionId } = useLogStore.getState();
    const lastTurn = turns[turns.length - 1];

    if (user && sessionId && lastTurn && lastTurn.isFinal) {
      const saveToDb = async () => {
        try {
          await chatHistoryService.saveMessage({
            ...
            role: lastTurn.role,
            text: lastTurn.text,
            is_final: lastTurn.isFinal,
            metadata: {
              timestamp: lastTurn.timestamp,
              groundingChunks: lastTurn.groundingChunks,
              toolResponse: lastTurn.toolResponse
            }
          });
        } catch (e) {
